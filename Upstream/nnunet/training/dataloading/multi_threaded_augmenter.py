...
def producer(queue, data_loader, transform, thread_id, seed, abort_event, task_pool, epoch_choice_id, lock, wait_time: float = 0.02):
    np.random.seed(seed)
...
class MultiThreadedAugmenter_dodnet(object):
    ...
    def _start(self):
        if not self.was_initialized:
            ...
            # print(self.task_pool, self.epoch_choice_id)
            with threadpool_limits(limits=1, user_api="blas"):
                for i in range(self.num_processes):
                    self._queues.append(Queue(self.num_cached_per_queue))
                    self._processes.append(Process(target=producer, args=(
                        self._queues[i], self.generator, self.transform, i, self.seeds[i], self.abort_event, self.task_pool, self.epoch_choice_id, self.lock)))
