...
class DataLoader3D_UniSeg(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3D_UniSeg, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
        ...
        self.pad_sides = pad_sides
        # self.data_shape, self.seg_shape = self.determine_shapes()
        print("Using UniSeg 3D Dataloader! with lock")

        self.task_id_choice = [int(i) for i in range(self.task_num)]


    def __next__(self, task_pool, epoch_choice_id, lock):
    ...
    def generate_train_batch(self, task_pool, epoch_choice_id, lock):
        ...
        with lock:
            ...
            if task_pool[choice_id] == 0:
                if len(epoch_choice_id) == 1:

                    for sub_class in range(self.task_num):
                        if sub_class == 0:
                            epoch_choice_id[sub_class] = sub_class
                        else:
                            epoch_choice_id.append(sub_class)
                        task_pool[sub_class] = 50
                else:
                    try:
                        epoch_choice_id.remove(choice_id)
                    except:
                        pass
        ...
        for j, i in enumerate(selected_keys):
            ...
            # print("1", selected_keys, case_all_data.shape, self.data_shape, self.seg_shape, self.patch_size)
            ...
            if seg_from_previous_stage is not None:
                seg_from_previous_stage = seg_from_previous_stage[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                          valid_bbox_y_lb:valid_bbox_y_ub,
                                          valid_bbox_z_lb:valid_bbox_z_ub]
            # print("2", selected_keys, case_all_data.shape, case_all_data[:-1].shape, self.pad_mode, self.pad_kwargs_data)
            data[j] = np.pad(case_all_data[:-1], ((0, 0),
                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                  (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
                             self.pad_mode, **self.pad_kwargs_data)
            # print("3", selected_keys, data[j].shape)
            seg[j, 0] = np.pad(case_all_data[-1:], ((0, 0),
                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
                                                    (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
            ...
            if seg_from_previous_stage is not None:
                seg[j, 1] = np.pad(seg_from_previous_stage, ((0, 0),
                                                             ...
                                                             (-min(0, bbox_z_lb),
                                                              max(bbox_z_ub - shape[2], 0))),
                                   'constant', **{'constant_values': 0})
        # print("4", selected_keys, data.shape)
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys}
